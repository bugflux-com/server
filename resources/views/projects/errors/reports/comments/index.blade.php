                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <div class="text-left font-normal mls mrs mts mbs" style="min-width: 200px; max-width: 100%">
                                    {{ Form::open('put', route('profile.notifications.reports.comments.update', $report->id)) }}

                                    <div class="form-field">
                                        {{ Form::label('new_report_comment', 'New comment') }}
                                        <div class="font-small font-neutral mbx"> Inform about new comments for this report. </div>

                                        <div>
                                            <span class="iblock mrm">
                                                {{ Form::hidden('new_report_comment.internal', 0) }}
                                                {{ Form::checkbox('new_report_comment.internal', 1, "Internal", $notifications->pivot && $notifications->pivot->internal) }}
                                            </span>
                                            <span class="iblock">
                                                {{ Form::hidden('new_report_comment.email', 0) }}
                                                {{ Form::checkbox('new_report_comment.email', 1, "E-mail", $notifications->pivot && $notifications->pivot->email) }}
                                            </span>
                                        </div>
                                    </div>
                ...
                <table class="table mts">
                            ...
                            <td class="text-nowrap text-top">
                                <div class="table-row-hover">
                                    @can('update', [$comment, $error->project_id])
                                        <a href="{{ route('projects.errors.reports.comments.edit', [$error->project_id, $error->id, $report->id, $comment->id]) }}"
                                    ...
                                    @can('delete', [$comment, $error->project_id])
                                        {{ Form::open('delete', route('projects.errors.reports.comments.destroy', [$error->project_id, $error->id, $report->id, $comment->id]), false, ['style' => 'display: inline']) }}
                    ...
                    @if($comments->currentPage() == $comments->lastPage() || $comments->total() == 0)
                        <tr>
                            ...
                            <td>
                                {{ Form::open('post', route('projects.errors.reports.comments.store', [$error->project_id, $error->id, $report->id])) }}
